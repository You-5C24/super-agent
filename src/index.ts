...
const toolSearchTool: ToolDefinition = {
  name: 'tool_search',
  description: '获取延迟工具的完整定义。传入工具名，返回完整参数 Schema',
  parameters: {
    type: 'object',
    properties: {
      query: { type: 'string', description: '工具名' },
    },
    required: ['query'],
  ...
  const simulatedTools: ToolDefinition[] = [
    {
      ...
      shouldDefer: true,
      searchHint: 'notion search pages',
      isConcurrencySafe: true,
      isReadOnly: true,
    ...
    },
    {
      ...
      description: '[MCP:supabase] 执行 SQL',
      parameters: {
        type: 'object',
      ...
      shouldDefer: true,
      searchHint: 'supabase sql query',
      isConcurrencySafe: true,
      isReadOnly: true,
      execute: async ({ sql }: any) => JSON.stringify([{ id: 1, sql }]),
    },
  ];
...
function injectFakeHistory(messages: ModelMessage[]) {
  const textOutput = (value: string) =>
    ({ type: 'text' as const, value }) satisfies {
      type: 'text';
      value: string;
    };
...
async function main() {
  ...
  console.log(
    `[压缩后] ${messages.length} 条消息, ~${afterSumTokens} tokens (节省 ${
      beforeTokens - afterSumTokens
    } tokens)\n`
  );
    ...
    rl.question('\nYou: ', async (input) => {
      ...
      if (currentTokens > 4000) {
        ...
        if (comp2.compressedCount > 0) {
          ...
          console.log(
            `  [Summarization] 压缩了 ${
              comp2.compressedCount
            } 条消息, ~${estimateTokens(messages)} tokens`
          );
        }
      }
