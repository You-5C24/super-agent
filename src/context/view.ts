...
export function renderContextLegend(snapshot: ContextSnapshot): string {
  ...
  lines.push(
    `${fmtTokens(usedTokens)}/${fmtTokens(windowTokens)} tokens (${pct(
      usedTokens,
      windowTokens
    )})`
  );
  ...
  for (const s of slices) {
    ...
    const value = `${fmtTokens(s.tokens)} tokens (${pct(
      s.tokens,
      windowTokens
    )})`;
  ...
  lines.push(
    `${fg(COLORS.free, '○')}  Free space: ${fmtTokens(
      Math.max(0, free)
    )} (${pct(Math.max(0, free), windowTokens)})`
  );
  lines.push(
    `${fg(COLORS.buffer, '▢')}  Autocompact buffer: ${fmtTokens(
      autocompactBufferTokens
    )} (${pct(autocompactBufferTokens, windowTokens)})`
  );
...
export function renderUsageView(tracker: UsageTracker): string {
  ...
  lines.push('');
  lines.push(
    `  ${C(111, '◎')} Input          ${fmtTokens(t.inputTokens).padStart(
      8
    )} tokens`
  );
  lines.push(
    `  ${C(220, '◈')} Cache write    ${fmtTokens(t.cacheWriteTokens).padStart(
      8
    )} tokens`
  );
  lines.push(
    `  ${C(36, '◉')} Cache read     ${fmtTokens(t.cacheReadTokens).padStart(
      8
    )} tokens   (${(t.hitRate * 100).toFixed(1)}% hit)`
  );
  lines.push(
    `  ${C(99, '◇')} Output         ${fmtTokens(t.outputTokens).padStart(
      8
    )} tokens`
  );
  lines.push('');
