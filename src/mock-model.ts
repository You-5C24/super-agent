...
function detectToolIntent(prompt: any[]): ToolCallIntent | null {
  ...
  if (
    ...
    const deletedOld =
      combined.includes('已删除: project_old-build-config') ||
      combined.includes('文件不存在: project_old-build-config');
    const deletedDup =
      combined.includes('已删除: project_deploy-process-2') ||
      combined.includes('文件不存在: project_deploy-process-2');
    if (combined.includes('lint 报告') && !deletedOld) {
      // Step 3: lint 完了，删掉 old-build-config
      return {
        toolName: 'memory',
        args: { action: 'delete', filename: 'project_old-build-config.md' },
      };
    }
    if (deletedOld && !deletedDup) {
      ...
      return {
        toolName: 'memory',
        args: { action: 'delete', filename: 'project_deploy-process-2.md' },
      };
    }
    // Step 5: done
    return null;
...
function pickTextResponse(prompt: any[]): string {
  ...
  if (hasToolResults(prompt)) {
    ...
    // Dream 完成——两条都处理完了
    if (
      (text.includes('dream') || text.includes('记忆整理')) &&
